...
- Caddy's HTTP/2 and HTTP/3 listeners are skipped because the listen address is plain `:8080` (no TLS); this is fine for the recruitment scope, where TLS termination would be done by the platform load balancer in a real deployment.
