...
- **No FK between `wallet_stocks.stock_name` and `bank_stocks(name)`.** A FK with `CASCADE` would wipe wallets on `POST /stocks`; without `CASCADE` the `TRUNCATE` would fail. Bank inventory and wallet holdings are kept independent in the schema; the application enforces the relationship.
...
- **`GET /wallets/{id}/stocks/{name}` returns 404 for a missing wallet, matching `GET /wallets/{id}`.** The spec specifies the success case as "returns a single number" but does not say what to do for an unknown wallet; mirroring the sibling endpoint keeps both reads consistent.
...
Together these tests verify two invariants: total stock is preserved across mixed buy/sell, and the audit log row count equals the number of successful operations.
