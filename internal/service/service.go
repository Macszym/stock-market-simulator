...
func (s *Service) SellStock(ctx context.Context, walletID, stockName string) error {
	if err := s.repo.SellStock(ctx, walletID, stockName); err != nil {
		return err
	}
	s.logger.Info("stock sold", "wallet_id", walletID, "stock_name", stockName)
	return nil
}
